<?php
	...
	else $retval = db_select("name, price, gid, imgsrc", "goods", "status=0 AND (TIMESTAMPDIFF(YEAR, goods.time, NOW()) < 5)");
	if ($retval->num_rows > 0) {
		...
		while ($row = $retval->fetch_assoc()) {
			...
			echo "<img src='../files/img/$fname' alt='image not found' width='180' height='180'>\n";
