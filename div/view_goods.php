<?php
	if (isset($_GET["keyword"])) {
		...
		$retval = db_select("name, price, gid", "goods", "status=0 AND type LIKE '%$keyword%'");
	}
	else $retval = db_select("name, price, gid", "goods", "status=0");
	if ($retval->num_rows > 0) {
		...
		while ($row = $retval->fetch_assoc()) {
			...
			// }
			echo "<div id='good_$n_wait_update' style='height:180px;width:180px;color:#000000'>wait for update</div>\n";
			echo "<div id='good_$n_price' style='font-size:30px;color:#ff0000'><b>$price</b></div>\n";
			echo "<div id='good_$n_name' style='color:#000000'>$goodname</div>\n";
			echo "</a>\n";
			$n++;
		}
		echo "</div>\n";
